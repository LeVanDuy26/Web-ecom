...
-- Reset AUTO_INCREMENT để customer mới bắt đầu từ id=2
ALTER TABLE nguoi_dung    AUTO_INCREMENT = 2;
...
BEGIN
  ...
  WHILE i <= 100 DO
    ...
    SET comment = CONCAT('Hàng quá chất lượng !', LPAD(i,3,'0'));
    SET dt = CONCAT(
      DATE_ADD('2024-01-01', INTERVAL FLOOR(RAND()*DATEDIFF(CURDATE(),'2024-01-01')) DAY),
      ' ', SEC_TO_TIME(FLOOR(RAND()*86400))
    );
