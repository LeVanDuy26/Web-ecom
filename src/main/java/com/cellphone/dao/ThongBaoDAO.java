...
import com.cellphone.model.ThongBao;
...
public class ThongBaoDAO {
    public List<ThongBao> getAll() {
        ...
        try (Connection conn = CSDLUtill.getConnection();
            ...
            while (rs.next()) {
                ...
                tb.setNgayBatDau(rs.getTimestamp("ngay_bat_dau"));
    ...
    public boolean insert(ThongBao tb) {
        String sql = "INSERT INTO thong_bao(tieu_de,noi_dung,ngay_bat_dau,ngay_ket_thuc) VALUES(?,?,?,?)";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, tb.getTieuDe());
            ps.setString(2, tb.getNoiDung());
            ps.setTimestamp(3, new Timestamp(tb.getNgayBatDau().getTime()));
            ps.setTimestamp(4, new Timestamp(tb.getNgayKetThuc().getTime()));
    ...
    public boolean update(ThongBao tb) {
        String sql = "UPDATE thong_bao SET tieu_de=?,noi_dung=?,ngay_bat_dau=?,ngay_ket_thuc=? WHERE id_thong_bao=?";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, tb.getTieuDe());
            ps.setString(2, tb.getNoiDung());
