...
public class SanPhamDAO {
    ...
    public boolean capNhatSanPham(SanPham sp) {
        ...
        try (Connection conn = CSDLUtill.getConnection();
            ...
            System.out.println("[DEBUG][DAO] Params: tenSanPham=" + sp.getTenSanPham() + ", moTa=" + sp.getMoTa() + ", gia=" + sp.getGia() + ", urlAnh=" + sp.getUrlAnh() + ", soLuongTon=" + sp.getSoLuongTon() + ", nhaSanXuat=" + sp.getNhaSanXuat() + ", id=" + sp.getIdSanPham());
            ps.setString(1, sp.getTenSanPham());
            ps.setString(2, sp.getMoTa());
            ps.setDouble(3, sp.getGia());
            ps.setString(4, sp.getUrlAnh());
            ...
            System.out.println("[DEBUG][DAO] Số dòng bị ảnh hưởng: " + affected);
            return affected > 0;
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @param soLuongCanBan Số lượng cần bán
     * @return true nếu đủ tồn kho, false nếu không đủ
     */
    public boolean kiemTraTonKho(int idSanPham, int soLuongCanBan) {
        String sql = "SELECT so_luong_ton FROM san_pham WHERE id_san_pham = ?";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    /**
     * Cập nhật tồn kho an toàn (chỉ cập nhật nếu đủ tồn kho)
     * @param idSanPham ID của sản phẩm
     * @param soLuongGiam Số lượng cần giảm
     * @return true nếu cập nhật thành công, false nếu không đủ tồn kho
     */
    public boolean capNhatTonKhoAnToan(int idSanPham, int soLuongGiam) {
        String sql = "UPDATE san_pham SET so_luong_ton = so_luong_ton - ? WHERE id_san_pham = ? AND so_luong_ton >= ?";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    /**
     * Hoàn trả tồn kho (tăng số lượng tồn kho)
     * @param idSanPham ID của sản phẩm
     * @param soLuongTang Số lượng cần tăng
     * @return true nếu cập nhật thành công
     */
    public boolean hoanTraTonKho(int idSanPham, int soLuongTang) {
        String sql = "UPDATE san_pham SET so_luong_ton = so_luong_ton + ? WHERE id_san_pham = ?";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
