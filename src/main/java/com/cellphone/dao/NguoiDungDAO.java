...
public class NguoiDungDAO {
    ...
    public boolean isPhoneExists(String sdt) {
        String sql = "SELECT COUNT(*) FROM nguoi_dung WHERE sdt = ?";
        try (Connection conn = CSDLUtill.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, sdt);
            ResultSet rs = ps.executeQuery();
    ...
    public boolean insertNguoiDung(NguoiDung nd) {
        ...
        if (isEmailExists(nd.getEmail())) {
            System.out.println("Email đã tồn tại: " + nd.getEmail());
            return false;
        }
        
        // Kiểm tra số điện thoại đã tồn tại
        if (isPhoneExists(nd.getSdt())) {
            System.out.println("Số điện thoại đã tồn tại: " + nd.getSdt());
            return false;
        }
